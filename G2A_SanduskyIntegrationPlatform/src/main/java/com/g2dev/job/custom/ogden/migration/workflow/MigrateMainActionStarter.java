...
import com.g2dev.job.custom.ogden.migration.sugar.Link_name_to_fields_array;
...
public class MigrateMainActionStarter {
	...
	public static void main(String[] args) {
		...
		// OgdenSugarSession.getInstance().printRecordCounts(".date_modified>20130000");
		...
		String[] modules2 = { "DocumentRevisions" };
		P_99_MigrateDefaults.main(modules2);
	...
	private static void test2() {
		...
		com.g2dev.job.custom.ogden.migration.sugar.SugarsoapPortType client = session
				.getSugar();
		// try {
	...
	public static class UpdateRelationships {
			...
			try {
				final int size = sugarSession
						...
						.get_entries_count(sugarSession.getSessionId(),
								getModuleName(), null, 0).getResult_count();
		...
		private void addRelationshipRecord(String id) {
			...
			// OgdenSugarSession.getInstance().printModuleRelationshipFields("Users");
			OgdenSugarSession session = OgdenSugarSession.getInstance();
			session.loginForOgden();
			...
			Link_field[] printModuleRelationshipFields = OgdenSugarSession
