...
public abstract class UpdateRelationshipsProcess {
	...
	public static String SanduskyQuery = null;
	...
	private static String findOldIdName(String moduleName) {
		return null;/*
					 * String fieldName = ""; SugarSession session =
					 * SugarSession .getInstance(); if (session.
					 * loginForSandusky ()) { try { Module_fields
					 * get_module_fields = session.getSugar ()
					 * .get_module_fields ( session.getSessionId (),
					 * moduleName); Field[] module_fields = get_module_fields .
					 * getModule_fields( ); for (Field field : module_fields) {
					 * if (field.getName( ).contains ("oldid")) { return
					 * field.getName(); } } } catch (RemoteException e) { //
	...
	private static void u01UpdateAccounts() {
		...
		// String query = "market_c='Sandusky'";
		String query = null;
		SanduskyQuery = "accounts.date_modified>20130000";
	...
	public static void updateModule(String moduleName,
			Map<String, String> relateFieldModule, List<String> relatedModules,
			String query) {
		List<String> moduleNames = new ArrayList<String>(relatedModules);
		moduleNames.add(moduleName);
		Map<String, Map<String, String>> moduleMap = loadModuleMap(moduleNames,
				query);
		SugarSession session = SugarSession.getInstance();
		if (session.loginForSandusky()) {
			SanduskySugarSession SanduskySession = SanduskySugarSession
					.getInstance();
			if (SanduskySession.loginForSandusky()) {
				List<String> fieldsSet = new ArrayList<String>(
						relateFieldModule.keySet());
				fieldsSet.add("id");
				String[] selectFields = fieldsSet.toArray(new String[fieldsSet
						.size()]);
				try {
					int batchSize = 500;
					int offset = 0;
					int numberOfEntries = SanduskySession
							.getSugar()
							.get_entries_count(SanduskySession.getSessionId(),
									moduleName, SanduskyQuery, 0)
							.getResult_count();

					long startTime = System.currentTimeMillis();
					while (numberOfEntries > (offset + batchSize)
							|| offset == 0) {
						com.g2dev.job.custom.sandusky.migration.sugar.Get_entry_list_result get_entry_list = SanduskySession
								.getSugar().get_entry_list(
										SanduskySession.getSessionId(),
										moduleName, SanduskyQuery, null,
										offset, selectFields, batchSize, 0);
						if (get_entry_list.getError() == null
								|| get_entry_list.getError().getNumber() == null
								|| get_entry_list.getError().getNumber()
										.equals("0")) {
							com.g2dev.job.custom.sandusky.migration.sugar.Entry_value[] entry_list = get_entry_list
									.getEntry_list();
							Name_value[][] updateRecords = new Name_value[entry_list.length][];
							for (int i = 0; i < entry_list.length; i++) {
								com.g2dev.job.custom.sandusky.migration.sugar.Entry_value entry_value = entry_list[i];
								com.g2dev.job.custom.sandusky.migration.sugar.Name_value[] name_value_list = entry_value
										.getName_value_list();
								Name_value[] updateNvList = new Name_value[name_value_list.length];
								for (int j = 0; j < name_value_list.length; j++) {
									com.g2dev.job.custom.sandusky.migration.sugar.Name_value name_value = name_value_list[j];
									Map<String, String> oldIdIdMap = null;
									if (name_value.getName().equalsIgnoreCase(
											"id")) {
										oldIdIdMap = moduleMap.get(moduleName);
									} else {
										oldIdIdMap = moduleMap
												.get(relateFieldModule
														.get(name_value
																.getName()));

										// if (updateNvList[j].getValue() ==
										// null
										// || updateNvList[j].getValue()
										// .isEmpty()) {
									...
									if (oldIdIdMap != null) {
										updateNvList[j] = new Name_value(
												name_value.getName(),
												oldIdIdMap.get(name_value
														.getValue()));
									}
								}
								updateRecords[i] = clearNullList(updateNvList);
							}
						...
						offset += batchSize;
						session.printETA(numberOfEntries, offset, startTime);
					}
				} catch (RemoteException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
