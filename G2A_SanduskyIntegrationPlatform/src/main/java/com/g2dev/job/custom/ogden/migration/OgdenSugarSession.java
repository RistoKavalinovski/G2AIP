package com.g2dev.job.custom.ogden.migration;
...
import com.g2dev.job.custom.ogden.migration.sugar.Get_entries_count_result;
...
import com.g2dev.job.custom.ogden.migration.sugar.Module_list;
...
import com.g2dev.job.custom.ogden.migration.sugar.New_module_fields;
import com.g2dev.job.custom.ogden.migration.sugar.New_set_entries_result;
import com.g2dev.job.custom.ogden.migration.sugar.Return_search_result;
import com.g2dev.job.custom.ogden.migration.sugar.SugarsoapLocator;
import com.g2dev.job.custom.ogden.migration.sugar.SugarsoapPortType;
import com.g2dev.job.custom.ogden.migration.sugar.User_auth;

public class OgdenSugarSession {

	private User_auth userAuth;
	private SugarsoapPortType sugar;
	private String sessionId;
	final static Logger logger = Logger.getLogger(OgdenSugarSession.class);

	private String username;
	private String password;
	private boolean forceLogin;

	public OgdenSugarSession() {
		super();
	}
	...
	private static OgdenSugarSession instance;

	public static OgdenSugarSession getInstance() {
		if (instance == null) {
			instance = new OgdenSugarSession();
		}
		return instance;
	}
	...
	public boolean loginForOgden() {
		// TODO check this
		return login("vkoper", "koper", "6.5.16");
	}

	public static void main(String[] args) {
		// OgdenSugarSession.getInstance().printModuleFields("Contacts");

		/*
		 * OgdenSugarSession instance2 = OgdenSugarSession.getInstance();
		 * instance2.loginForOgden(); String module_name = "Contacts"; String[]
		...
		getInstance().printModuleFields("Opportunities");
	...
	public static class UndeleteEntriesJob {
		...
		public void start() {
			...
			selectFields.add("id");
			OgdenInput ogdenInput = new OgdenInput(getModuleName(),
					selectFields);
			ogdenInput.setBatchSize(getBatchSize());
			ogdenInput.setDeleted(1);
			ogdenInput.start(getQuery());
			// Get_entry_list_result nextBatch = null;
			// while ((nextBatch = ogdenInput.nextBatch(getQuery())) != null) {
			...
			try {
				...
				for (Module_list_entry string : modules) {
					Get_entries_count_result get_entries_count = getSugar()
							.get_entries_count(
									getSessionId(),
									string.getModule_key(),
									string.getModule_key().toLowerCase()
